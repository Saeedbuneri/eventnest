...
export default function AboutPage() {
  return (
    <div className="min-h-screen bg-[#07070e]">
      ...
      {/* Hero */}
      <section className="bg-[#0a0a10] text-white py-14 sm:py-24 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4">
          ...
          <p className="text-gray-400 text-base sm:text-xl max-w-2xl leading-relaxed">
      ...
      {/* Story */}
      <section className="max-w-3xl mx-auto px-4 py-12 sm:py-20">
        <p className="text-xs font-bold tracking-[0.2em] text-brand-500 uppercase mb-3">Our Story</p>
        <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-6">From a problem to a platform.</h2>
      ...
      {/* Timeline */}
      <section className="bg-[#0c0c18] py-16">
        <div className="max-w-3xl mx-auto px-4">
          <p className="text-xs font-bold tracking-[0.2em] text-brand-500 uppercase mb-3">Timeline</p>
          <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-10">How we got here.</h2>
          <div className="space-y-6">
            {MILESTONES.map((m) => (
              <div key={m.year} className="flex gap-5">
                <div className="w-14 shrink-0 text-right">
                  <span className="text-sm font-black text-brand-400">{m.year}</span>
                </div>
                <div className="flex-1 border-l-2 border-brand-500/30 pl-5 pb-6">
      ...
      <section className="max-w-5xl mx-auto px-4 py-20">
        ...
        <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-10">What we stand for.</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {VALUES.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-[#111118] rounded-2xl border border-white/[.08] p-6 hover:border-white/[.14] transition-all">
      ...
      <section className="bg-[#0c0c18] py-16">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-10">People behind the platform.</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {TEAM.map((m) => (
              <div key={m.name} className="bg-[#111118] rounded-2xl border border-white/[.08] p-6 text-center hover:border-white/[.14] transition-all">
                <div className="w-16 h-16 bg-brand-500/10 rounded-full flex items-center justify-center text-brand-400 font-black text-2xl mx-auto mb-4">
                  {m.avatar}
                </div>
                <p className="font-bold text-white">{m.name}</p>
                <p className="text-xs text-brand-400 font-semibold mb-2">{m.role}</p>
