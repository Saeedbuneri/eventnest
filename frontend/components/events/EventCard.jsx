...
export default function EventCard({ event }) {
  ...
  return (
    <Link href={`/events/${event._id}`} className="card group flex flex-col overflow-hidden">
      {/* Banner */}
      ...
      <div className="p-4 flex flex-col flex-1 gap-2">
        <h3 className="font-bold text-gray-900 leading-snug group-hover:text-brand-600 transition-colors line-clamp-2">
          {event.title}
        </h3>

        <p className="text-gray-500 text-sm line-clamp-2">{truncate(event.description, 100)}</p>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span className="text-xs text-gray-400">{soldPct}% sold</span>
        </div>
        ...
        <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${
              almostFull ? 'bg-red-400' : 'bg-brand-400'
            }`}
            style={{ width: `${soldPct}%` }}
          />
        </div>
